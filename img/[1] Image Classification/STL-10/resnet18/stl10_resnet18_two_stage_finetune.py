...
# -----------------------------
def get_transforms():
    # ImageNet pretrained ResNet18 expects ImageNet-style normalization.
...
def unfreeze_layer4_and_fc(model):
    # ResNet18 blocks: layer1, layer2, layer3, layer4.
...
def plot_curves(history, output_dir):
    ...
    plt.ylabel("Loss")
    plt.title("STL-10 ResNet18 Two-Stage Fine-tuning Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel("Global Epoch")
    plt.ylabel("Accuracy")
    plt.title("STL-10 ResNet18 Two-Stage Fine-tuning Accuracy")
    plt.legend()
    plt.grid(True)
...
def main(args):
    ...
    best_model_path = os.path.join(args.output_dir, "best_resnet18_two_stage.pth")
    ...
    last_model_path = os.path.join(args.output_dir, "last_resnet18_two_stage.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./outputs_resnet18_two_stage")
