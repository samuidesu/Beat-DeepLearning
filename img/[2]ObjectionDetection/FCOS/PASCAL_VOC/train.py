"""Training entry point for FCOS on PASCAL VOC.

Two-stage finetuning (identical protocol to the YOLO3 project, so results are
directly comparable):
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch_desc=""):
    ...
    # them). Only the backbone has BN; the FCOS head uses GroupNorm.
    ...
    for images, targets in tqdm(loader, desc=epoch_desc, leave=False):
        ...
        # Forward -> 3 raw prediction tensors; loss assigns targets + reduces.
        preds = model(images)
...
def evaluate(model, loader, criterion, device, epoch_desc="eval"):
    ...
    # tracks the (fast) validation loss for monitoring.
    model.eval()
    running, seen = {}, 0
    for images, targets in tqdm(loader, desc=epoch_desc, leave=False):
        images = images.to(device, non_blocking=True)
...
def run_stage(stage_id, model, train_loader, val_loader, criterion, optimizer,
    ...
    for e in range(1, epochs + 1):
        ...
        # we actually care about; val loss alone can disagree with it (the
        # YOLO3 experiments showed them decoupling late in training).
...
def plot_curves(history, output_dir):
    ...
    Produces:
        ...
        loss_components.png  - train cls/reg/ctr losses per epoch.
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("FCOS loss components (train)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
# -----------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Train FCOS on PASCAL VOC")
...
def main():
    ...
    model = FCOS(num_classes=config.NUM_CLASSES,
                 ...
                 num_head_convs=config.NUM_HEAD_CONVS,
                 cls_prior=config.CLS_PRIOR).to(device)
    # .to(device) moves the loss's regression-range buffer onto the GPU too;
    # without this it stays on CPU and causes a device-mismatch on CUDA/MPS.
    criterion = FCOSLoss().to(device)
