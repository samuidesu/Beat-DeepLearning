"""PASCAL VOC dataset for FCOS.

Responsibilities of this file:
  1. Download VOC into DATA_ROOT (via torchvision) -- or transparently reuse
     the copy the YOLO3 project already downloaded (see config.DATA_ROOT).
...
The FCOS target assignment (which pyramid location is responsible for each
box) is intentionally NOT done here -- it happens in the loss, so all
assignment logic lives in one place. (Same split of responsibilities as the
YOLO3 project, where the anchor matching lives in its loss.)
...
def download_voc():
    """Download the VOC splits we need into DATA_ROOT.
    ...
    NOTE: if config.DATA_ROOT resolved to the YOLO3 project's data folder,
    torchvision will detect the existing files and skip the download.
    """
