"""Central configuration for U-Net semantic segmentation on PASCAL VOC 2012.

Semantic segmentation = classify EVERY PIXEL into one of NUM_CLASSES classes.
This project is the FCN project with the neck swapped for a U-Net decoder:
a pretrained ResNet is the contracting path (encoder), a chain of
upsample + concat-skip + DoubleConv blocks is the expanding path (decoder),
and a 1x1 classifier head predicts 21 logits per pixel at stride 2 before a
final 2x upsample. Everything else -- data pipeline, loss, two-stage finetune
schedule, metrics -- is identical to the FCN project, so any mIoU difference
is attributable to the architecture.
"""
...
# lists), so a copy any earlier project downloaded serves this project as-is.
# Candidates are probed IN ORDER: the sibling FCN project first (so the two
# segmentation projects share one copy -- including its sbd/ subfolder if SBD
# was downloaded there), then the detection projects' copies; if none has
# VOC2012, data is downloaded into this project's own dataset/data/
# (gitignored).
_LOCAL_DATA = os.path.join(PROJECT_ROOT, "dataset", "data")
_FCN_DATA = os.path.normpath(os.path.join(
    PROJECT_ROOT, "..", "FCN", "dataset", "data"))
...
for _cand in (_FCN_DATA, _YOLO3_DATA, _FCOS_DATA, _LOCAL_DATA):
...
# Extra training data: SBD ("VOC aug") adds ~9k VOC images that only have
# segmentation labels in the SBD release (train grows 1464 -> ~10.5k images)
# and typically lifts mIoU by several points. ON here (same setting as the
# FCN run we compare against). If <DATA_ROOT>/sbd/ is not already present it
# is downloaded once (~1.4 GB; reading its .mat masks needs scipy). See
# dataset/voc.py (SBDSegDataset) and the README.
USE_SBD = True
...
# Model
# -----------------------------------------------------------------------------
# Encoder architecture: "resnet18" or "resnet34" (same options as the FCN /
# detection projects; both tap C1..C5 at channels 64/64/128/256/512).
# resnet34 = the FCN run's choice, kept for comparability.
BACKBONE = "resnet34"

# (No FPN_CHANNELS knob anymore: the U-Net decoder's widths mirror the
# encoder's tap channels instead of using one constant pyramid width.)
