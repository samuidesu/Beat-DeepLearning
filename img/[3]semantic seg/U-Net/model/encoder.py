"""Encoder: the contracting path of the U-Net (a pretrained ResNet).

Same idea as the FCN project's backbone -- a torchvision ResNet pretrained on
ImageNet -- but used as the U-Net ENCODER: every intermediate resolution is
tapped and handed to the decoder as a skip connection. The classic U-Net
trained its encoder from scratch; reusing a pretrained classifier here gives
the decoder much stronger features for free.

One tap MORE than the FCN backbone: C1, the stride-2 output of the stem's
conv+bn+relu (taken BEFORE the maxpool). The FCN neck stopped fusing at
stride 4; the U-Net decoder climbs back to stride 2, and it needs an encoder
map at that resolution to concatenate with.

ResNet layout (for a 416x416 RGB input) and where we tap features:

    input                 [B, 3, 416, 416]
    conv1+bn+relu -> C1   [B, 64, 208, 208]   stride 2   <- tap (NEW vs FCN)
...
C1/C2/C3/C4 are the skip connections; C5 is the bottleneck the decoder
starts from.
"""
...
# Supported backbones -> (constructor, weights enum, (C1..C5) tap channels).
# ResNet-18 and -34 both use BasicBlock, so their tap channels are identical
# (64/64/128/256/512); only the number of blocks per stage differs. (ResNet-50+
# use Bottleneck -> stage channels 256/512/1024/2048 while C1 stays 64; add here
# with the right channels to support them.)
_RESNET_ARCHS = {
    "resnet18": (resnet18, ResNet18_Weights, (64, 64, 128, 256, 512)),
    "resnet34": (resnet34, ResNet34_Weights, (64, 64, 128, 256, 512)),
}


class ResNetBackbone(nn.Module):
    """ResNet encoder (resnet18 / resnet34) -> 5 multi-scale feature maps.

    Args:
        arch (str): which ResNet, "resnet18" or "resnet34". Both share the same
            tap channels, so the decoder/head need no change when switching.
        pretrained (bool): if True, load ImageNet-pretrained weights.
        freeze (bool): if True, freeze all encoder parameters (stage-1 of a
            two-stage finetune: train only decoder/head first, unfreeze later --
            the same recipe that drove the FCOS project's mAP gain).

    Attributes:
        out_channels (tuple[int, ...]): channel counts of (C1, C2, C3, C4, C5),
            e.g. (64, 64, 128, 256, 512). The decoder reads this to size its
            UpBlocks.
        strides (tuple[int, ...]): the strides (2, 4, 8, 16, 32) of
            (C1, C2, C3, C4, C5).
    """

    # Strides of the 5 taps (fixed by the ResNet layout). out_channels depends
    ...
    def forward(self, x: torch.Tensor):
        ...
        Input:
            x: image batch, shape [B, 3, H, W] (H, W MUST be multiples of 32,
               otherwise the decoder's 2x upsamples won't line up with the
               skip maps for the channel-wise concat).

        Output:
            (c1, c2, c3, c4, c5): tuple of 5 feature maps
                c1: [B, 64,  H/2,  W/2]   (stride 2,  stem output, finest detail)
        ...
        """
        # Stem conv (stride 2): first tap C1 -- BEFORE the maxpool, so the
        # decoder gets a stride-2 skip map to recover fine boundaries with.
        x = self.conv1(x)
        x = self.bn1(x)
        c1 = self.relu(x)         # [B, 64, H/2, W/2]
...
if __name__ == "__main__":
    ...
    c1, c2, c3, c4, c5 = model(dummy)
    print("c1:", tuple(c1.shape), "(expected (2, 64, 208, 208))")
