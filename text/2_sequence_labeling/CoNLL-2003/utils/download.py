"""Resumable multi-connection downloader (shared by the CoNLL-2003 and GloVe fetch).

Copied from the IMDB project, itself ported from the segmentation projects'
dataset/voc.py, where it existed for one reason: the hosts we depend on
throttle PER CONNECTION, so from a far-away network a single stream crawls and
a 862 MB GloVe archive takes hours. Opening N parallel HTTP byte-range
requests multiplies the throughput by roughly N, and because each segment is
written in append mode, a dropped connection (or a Ctrl-C) costs only the
bytes still missing -- rerunning resumes.

One deletion relative to the IMDB copy: extract_tar is gone. CoNLL-2003 ships
as three plain text files, not an archive, so the only extractor still needed
is extract_zip -- and that is only for GloVe, which normally is not
downloaded at all (config.GLOVE_PATH finds the copy the classification
projects already fetched).
"""
