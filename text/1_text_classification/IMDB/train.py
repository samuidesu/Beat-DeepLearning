"""Training entry point for RNN and Transformer sentiment classification on IMDB.
...
This is the LONG-DOCUMENT member of the text trio: the median review is 230
tokens against AG News' 44 and SST-2's 7. Everything below is unchanged from
the AG-News sibling except the corpus module and a few measured constants,
which is the point -- if the cells rank differently here, it is the sequence
length doing it and not a quietly retuned hyperparameter.
...
WHAT IS EVALUATED: the 2,500-review validation split carved out of train,
NOT test.csv. IMDB publishes its test labels, so scoring test every epoch and
then keeping the best epoch would quietly turn the "test accuracy" into a
...
Usage:
    ...
    python train.py --download             # fetch IMDB + GloVe first
...
def build_dataloaders(batch_size, num_workers, download, device, use_glove,
                      truncation=None, head_len=None):
    """Build the train / val dataloaders and the vocabulary.

    Train = 22,500 reviews (90% of train.csv), shuffled.
    Val   =  2,500 reviews (the held-out 10%), in order, larger batches
            (no backward pass to fit in memory).
    Both use collate_batch, which pads each batch to ITS OWN longest review
    -- which, measured, saves almost nothing on this corpus (18% of reviews
    hit MAX_LEN, so nearly every shuffled batch is 400 wide anyway). See
    collate_batch's docstring for the numbers and for what would help.
    ...
    if download or not imdb_present():
        download_imdb()
    if use_glove and (download or not glove_present()):
        download_glove()

    vocab = build_vocab_from_train()
    # Same truncation on both splits, obviously -- but note the vocabulary is
    # built from UNTRUNCATED text (build_vocab_from_train), so switching
    # --truncation does not change a single word id. That is what makes the
    # head vs head_tail comparison a clean one.
    train_set = IMDBDataset("train", vocab, truncation=truncation,
                            head_len=head_len)
    val_set = IMDBDataset("val", vocab, truncation=truncation,
                          head_len=head_len)
...
def evaluate(model, loader, criterion, device, epoch_desc="eval"):
    ...
    The whole val split (2,500 reviews) is scored every epoch, so the
...
def collect_run_meta(args, device, train_loader, val_loader, vocab, *,
    ...
    meta = {
        ...
        "data": {
            ...
            "truncation": train_loader.dataset.truncation,
            "head_len": train_loader.dataset.head_len,
            "label_counts": train_loader.dataset.label_counts(),
            "val_unk_rate": round(val_loader.dataset.unk_rate(), 4),
            "train_truncated_rate": round(train_loader.dataset.truncated_rate(), 4),
        },
...
def plot_curves(history, output_dir, title_prefix="RNN"):
    ...
    plt.xlabel("epoch")
    plt.ylabel("metric")
    # Zoomed y-range, but only down to chance (0.50) rather than AG News'
    # 0.70: a vanilla RNN reading 400 tokens with "last" pooling can genuinely
    # sit near chance for several epochs, and a clipped curve would hide the
    # single most interesting failure this project is set up to show.
    plt.ylim(0.5, 1.0)
...
# -----------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Train a sentiment classifier on IMDB")
    ...
    p.add_argument("--download", action="store_true",
                   help="download IMDB (and GloVe) before training")
    ...
    p.add_argument("--hidden-size", type=int, default=config.HIDDEN_SIZE,
                   help="RNN hidden width PER DIRECTION. The counterpart of "
                        "--dim on the Transformer side; use it with "
                        "--output-dir for a capacity ablation")
    ...
    p.add_argument("--norm", choices=["pre", "post"],
                   ...
                   help="Transformer LayerNorm placement. post = LN(x+f(x)), "
                        "the 2017 original, needs LR warmup once it gets deep; "
                        "pre = x+f(LN(x)), trains without warmup. Ignored by "
                        "--model rnn")
    ...
    p.add_argument("--truncation", choices=["head", "head_tail"],
                   ...
                   help="which end(s) of a too-long review to keep. head_tail "
                        "keeps --head-len from the front plus the rest from "
                        "the back, because an IMDB verdict often lands last")
    p.add_argument("--head-len", type=int, default=config.HEAD_LEN,
                   help="tokens kept from the front under --truncation "
                        "head_tail; the remaining MAX_LEN - head_len come from "
                        "the end. 100 reproduces the 25/75 split Sun et al. "
                        "found best for BERT on this corpus")
    ...
    if not 0 <= args.head_len <= config.MAX_LEN:
        p.error(f"--head-len must be in [0, MAX_LEN={config.MAX_LEN}]")
...
def main():
    ...
    train_loader, val_loader, vocab = build_dataloaders(
        ...
        truncation=args.truncation, head_len=args.head_len)
    print(f"Train docs: {len(train_loader.dataset)}  ({len(train_loader)} batches)  "
          f"Val docs: {len(val_loader.dataset)}  Vocab: {len(vocab)}")
    cut = (f"head {args.head_len} + tail {config.MAX_LEN - args.head_len}"
           if args.truncation == "head_tail" else f"first {config.MAX_LEN}")
    print(f"Truncated at max_len={config.MAX_LEN} ({cut}): "
          f"{train_loader.dataset.truncated_rate():.2%} of training reviews")
    ...
    if is_transformer:
        ...
        model_cfg.update(hidden_size=args.hidden_size, cell=args.cell,
    ...
    # Plain cross-entropy: one prediction per review, no positions to
    # ignore, no auxiliary heads -- so there is nothing for a Loss class to
    # wrap. Label smoothing replaces the hard target (1, 0) with (1-eps, eps):
    # being right is still rewarded, being *certain* is not. Class weights are
    # pointless here -- IMDB is exactly balanced. nn.CrossEntropyLoss takes
    # RAW logits (log_softmax is applied internally), which is why the model
    # never ends with a softmax.
    ...
    if not model.embedding.pretrained_loaded and args.epochs_stage1 > 0:
        print("[imdb] WARNING: no GloVe vectors were loaded but stage 1 will "
