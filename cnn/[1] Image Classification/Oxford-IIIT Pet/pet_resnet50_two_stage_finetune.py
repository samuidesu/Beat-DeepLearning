...
# -----------------------------
def get_transforms():
    # ImageNet pretrained ResNet50 expects ImageNet-style normalization.
...
def build_dataloaders(args, device):
    ...
    full_train_set = datasets.OxfordIIITPet(
        root=args.data_dir,
        split="trainval",
    ...
    full_train_set_eval = datasets.OxfordIIITPet(
        root=args.data_dir,
        split="trainval",
        target_types="category",
        download=False,
...
def unfreeze_layer4_and_fc(model):
    # ResNet50 blocks: layer1, layer2, layer3, layer4.
    # layer4 is the last residual block group, closest to the classifier head.
    freeze_all(model)
...
def get_lr_dict(optimizer):
    # Round to 5 decimal places for cleaner log output.
    return {
        group.get("name", f"group_{i}"): round(group["lr"], 5)
...
def plot_curves(history, output_dir):
    ...
    plt.ylabel("Loss")
    plt.title("Oxford-IIIT Pet ResNet50 Two-Stage Fine-tuning Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel("Global Epoch")
    plt.ylabel("Accuracy")
    plt.title("Oxford-IIIT Pet ResNet50 Two-Stage Fine-tuning Accuracy")
    plt.legend()
    plt.grid(True)
...
def main(args):
    ...
    config = {
        ...
        "stage2_layer4_lr": args.layer4_lr,
    ...
    best_model_path = os.path.join(args.output_dir, "best_resnet50_two_stage.pth")
    stage1_best_path = os.path.join(args.output_dir, "stage1_best_head_only.pth")
    stage2_best_path = os.path.join(args.output_dir, "stage2_best_layer4_fc.pth")
    last_model_path = os.path.join(args.output_dir, "last_resnet50_two_stage.pth")
    ...
    optimizer_stage1 = optim.AdamW(
        [
            {"params": model.fc.parameters(), "lr": args.head_lr, "name": "fc"},
        ],
        weight_decay=args.weight_decay,
    )
    ...
    # Stage 2: unfreeze the last ResNet block group layer4 + MLP head, use smaller LRs.
    unfreeze_layer4_and_fc(model)
    optimizer_stage2 = optim.AdamW(
        [
            {"params": model.layer4.parameters(), "lr": args.layer4_lr, "name": "layer4"},
            {"params": model.fc.parameters(), "lr": args.head_ft_lr, "name": "fc"},
        ],
        weight_decay=args.weight_decay,
    )
    ...
    best_val_acc, global_epoch = run_stage(
        stage_name="stage2_layer4_plus_head",
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        criterion=criterion,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./resnet50")
    ...
    # Stage 2: unfreeze layer4 + head, then fine-tune with smaller LRs.
